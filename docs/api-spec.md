...
| 名前         | 型              | 必須 | 説明                                    |
|--------------|----------------|------|-----------------------------------------|
| `q`          | string (0-200) | ❌   | 検索キーワード（タイトル+本文の部分一致）    |
...
- 前後空白はサーバー側でトリムされ、空白のみは「未指定」と同等（フィルタなし）に扱う
- SQL の `ILIKE` メタ文字である `%` / `_` / `\` は **リテラル** として扱う（クライアントがワイルドカードを組み立てる必要はなく、両端の部分一致はサーバー側で付与する）
- 例: `q=100%` は文字列 `"100%"` を含むメモにマッチし、`q=user_a` は文字列 `"user_a"` を含むメモにのみマッチする
- 大文字小文字は区別しない
...
| `body`        | string (0-10000) \| null | ❌   | 本文。末尾の空白・改行はサーバー側でトリム、空文字列・空白のみは `null` に正規化。NULL バイトを含む場合は 400 |
| `tag_ids`     | string[]       | ❌   | 既存タグIDの配列（最大10件）          |
| `is_pinned`   | boolean        | ❌   | デフォルト `false`                  |

**`body` の挙動**

- 末尾の空白・改行のみサーバー側で除去（先頭・中間の空白は保存）
- トリム後に空になった入力は `null` に正規化
- NULL バイト (`\x00`) を含む入力は `400 VALIDATION_ERROR`（`null_byte_in_body`）として拒否
- TAB / LF / CR を含むそれ以外の制御文字は許容
...
**エラー**
- `400 VALIDATION_ERROR`: タイトル未入力、文字数超過、`body` の NULL バイト混入 など
